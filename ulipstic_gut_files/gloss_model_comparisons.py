...
from GlossPath.regressor import Regressor
...
prepped_data_kegg = PrepData(datapath, 'kegg', 'avg_sample_hto', 'total_counts', 'raw_biotin',)

with open('./experiment_evaluations/gut_glosspath_tuning_kegg_oct9_2024.pickle', 'rb') as handle:
    kegg_g_cv = pickle.load(handle)
    
with open('./experiment_evaluations/gut_glosspath_tuning_hallmark_oct8_2024.pickle', 'rb') as handle:
    hallmark_g_cv = pickle.load(handle)
    
with open('../model_ulipstic_gut/experiment_evaluations/gloss_lasso_tuning_gut_oct23_2024.pickle', 'rb') as handle:
    lasso_dict = pickle.load(handle)
    
with open('../model_ulipstic_gut/experiment_evaluations/gloss_elasticnet_tuning_gut_oct23_2024.pickle', 'rb') as handle:
    enet_dict = pickle.load(handle)
    
with open('../model_ulipstic_gut/experiment_evaluations/gloss_ridge_tuning_gut_oct23_2024.pickle', 'rb') as handle:
...
for res in resolutions:
    ...
    for ctype in resolutions[res]:
        ...
        for rand in range(50):
            ...
            ridge_reg = Ridge(alpha = optimal_ridge_params[res][ctype]).fit(x_ftrain, y_ftrain)

            ridge_ftrain_res.append(r2(ridge_reg, x_ftrain, y_ftrain))
            ridge_ftest_res.append(r2(ridge_reg, x_ftest, y_ftest))

            elasticnet_reg = ElasticNet(alpha = optimal_enet_params[res][ctype][1], l1_ratio=optimal_enet_params[res][ctype][0]).fit(x_ftrain, y_ftrain)

            elasticnet_ftrain_res.append(r2(elasticnet_reg, x_ftrain, y_ftrain))
            elasticnet_ftest_res.append(r2(elasticnet_reg, x_ftest, y_ftest))

            lasso_reg = skglm.Lasso(alpha = optimal_lasso_params[res][ctype]).fit(x_ftrain, y_ftrain)

            lasso_ftrain_res.append(r2(lasso_reg, x_ftrain, y_ftrain))
            lasso_ftest_res.append(r2(lasso_reg, x_ftest, y_ftest))
            
        methods_test = {
            'hallmark' : hallmark_test_res,
            'kegg' : kegg_test_res,
            'lasso' : lasso_ftest_res,
            'ridge' : ridge_ftest_res,
            'elasticnet' : elasticnet_ftest_res
        }
        
        methods_train = {
            'hallmark' : hallmark_train_res,
            'kegg' : kegg_train_res,
            'lasso' : lasso_ftrain_res,
            'ridge' : ridge_ftrain_res,
            'elasticnet' : elasticnet_ftrain_res
        }
        
        comp_res[res][ctype] = [ methods_test, methods_train ]

with open('./experiment_evaluations/model_arena_oct23_2024.pickle', 'wb') as handle:
